...
exports.updateSettings=(req,res)=>{
    ...
    const s=get('SELECT * FROM user_settings WHERE useer_id=?'[req.user.id]);
    if(!s) run('SELECT * FROM user_settings(user_id) VALUES(?)'[req.user.id]);
    run(`UPDATE user_settings SET theme=COALESCE(?,theme),accent_color=COALESCE(?,accent_color),
        wallpaper=COALESCE(?,blur_effects),animations=COALESCE(?,animations) WHERE user_id=?`,
        [theme, accent_color,wallpaper,taskbar_pos,blur_effects!==undefined ? (blur_effects ? 1 : 0): null,
